...
(function () {
  ...
  // FIX 2026-09-20：修复版（诊断基线构建 ts=1789861143175 之后）启动时清一次错误环里
  // 修复前遗留的历史条目——maybeCardLockReminder 未定义（#470 已修）、音乐 blob 音频
  // 误采集（本次 data-mochi-audio 补丁）、旧版屏幕适配瞬态快照。错误环跨刷新保留 20 条，
  // 不清则升级后诊断结论仍亮「发现 1 项：最近错误 20 条」（mochi-diag-20260920 实锤）。
  // 只删时间戳早于修复基线的条目，新版本上新发生的错误照常保留；按时间戳过滤天然幂等，
  // 无需旗标，后续启动均为空操作，不会误清日后的新线索。idb.js 在本文件之后执行，
  // 首跑时 idbGet 尚未就绪，IDB 副本等 mochi-restore-done 再清一次。
  const PRE_FIX_PURGE_TS = 1789861143175;
  function filterPreFixErrs(raw) {
    let arr = null;
    try {
      if (raw) {
        const o = JSON.parse(raw);
    ...
    const kept = arr.filter(function (it) {
      ...
      return t === 0 || t > PRE_FIX_PURGE_TS; // 无时间戳的异常条目保守保留
    });
    return kept.length === arr.length ? null : JSON.stringify(kept);
  }
  function purgePreFixDiagErrs() {
    try {
      const sLS = filterPreFixErrs(localStorage.getItem(ERR_KEY));
      if (sLS !== null) {
        try { localStorage.setItem(ERR_KEY, sLS); } catch (e1) {}
        try { refreshBadge(); } catch (e2) {}
      }
    } catch (e3) {}
    if (window.idbGet && window.idbSet) {
      try {
        window.idbGet(ERR_KEY).then(function (raw) {
          const sIDB = filterPreFixErrs(typeof raw === 'string' ? raw : (raw ? JSON.stringify(raw) : ''));
          if (sIDB !== null) return window.idbSet(ERR_KEY, sIDB);
        }).then(function () { try { refreshBadge(); } catch (e4) {} }).catch(function () {});
      } catch (e5) {}
    }
  }
    ...
    window.addEventListener('error', function (e) {
      ...
      try {
        if (e && e.message) {
          ...
          // FIX 2026-09-20：music-player.js 自建并打了 data-mochi-audio 标记的音频
          //（本地上传探测 / 播放实例），其失败已在模块内自行处理（probeBad 计数、
          // blob↔dataURL 互备、toast 提示）；尤其切歌清场 removeAttribute('src')+load()
          // 中断 blob 加载也会派发 error，元素 onerror 置空拦不住 window 捕获。
          // 这些预期事件不进错误环（诊断实锤：9/17 page-music 6 条 blob 全是它）。
          if (tag === 'audio' || tag === 'source') {
            try { if (e.target.closest && e.target.closest('[data-mochi-audio]')) return; } catch (e5) {}
          }
  ...
  // FIX 2026-09-20：清修复版部署前遗留的历史误报；首跑后 IDB 回填完成再清一次副本
  try { purgePreFixDiagErrs(); } catch (e) {}
  try { document.addEventListener('mochi-restore-done', function () { try { purgePreFixDiagErrs(); } catch (e) {} }); } catch (e) {}
  try { refreshBadge(); } catch (e) {}
