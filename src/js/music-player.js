...
(function () {
  ...
  function uploadFiles(files) {
    ...
    const next = function () {
      ...
      readFile(file, function (buf) {
        ...
        // FIX 2026-09-20：模块内已自行处理探测失败（probeBad 计数/「放不了」徽标），
        // 打标记让 device.js 全局错误采集跳过，预期失败不再刷进诊断错误环
        try { tmp.setAttribute('data-mochi-audio', 'music-upload-probe'); } catch (e) {}
        tmp.preload = 'metadata';
        let tmpUrl = null;
        let metaTimer = null;
        let settled = false;
  ...
  function createAudio() {
    ...
    liveAudioEls = [];
    const a = new Audio();
    // FIX 2026-09-20：播放器自建音频打标记。切歌清场 killAudioEl 会对旧元素
    // removeAttribute('src')+load()，中断 blob 加载时浏览器仍派发 error 事件
    //（onerror 置空也拦不住 window 捕获阶段监听），本模块的失败/兜底逻辑已自洽，
    // 这些预期事件不应进 device.js 诊断错误环刷假红点（9/17 6 条 blob 误报实锤）。
    try { a.setAttribute('data-mochi-audio', 'music-player'); } catch (e) {}
